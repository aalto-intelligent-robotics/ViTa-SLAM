...
ExperienceMap::ExperienceMap(ptree settings)
{
    ...
    accum_delta_gamma = EXP_INITIAL_EM_DEG * M_PI/180;
    accum_delta_x = 0;
    accum_delta_y = 0;
...
/*! Update the current position in the experience map since the last experience */
void ExperienceMap::on_odo(double vtrans_x, double vtrans_y, double vrot, double time_diff_s)
{
    // Split the translational velocities in x and y component
    vtrans_x = vtrans_x * time_diff_s;
    vtrans_y = vtrans_y * time_diff_s;
    vrot = vrot * time_diff_s;
    accum_delta_gamma = clip_rad_180(accum_delta_gamma + vrot);

    // Comput the contribution of each velocity to the position change
    double x_bla_x = vtrans_x * cos(accum_delta_gamma);
    double x_bla_y = vtrans_x * sin(accum_delta_gamma);

    double y_bla_x = vtrans_y * sin(accum_delta_gamma);
    double y_bla_y = vtrans_y * cos(accum_delta_gamma);

    accum_delta_x = accum_delta_x + x_bla_x - y_bla_x;
    accum_delta_y = accum_delta_y + x_bla_y + y_bla_y;
...
bool ExperienceMap::calculate_path_to_goal(double time_s)
{
    ...
    if (exp_euclidean_m(&experiences[current_exp_id], &experiences[goal_list[0]]) < 0.1 || ((goal_timeout_s != 0) && time_s > goal_timeout_s))
    {
        ...
        {
            cout << "Timed out reaching goal ..." << endl;
            goal_success = false;
        }
...
{
    if (goal_list.size() == 0) return false;
    waypoint_exp_id = -1;
    double dist;
    unsigned int trace_exp_id = goal_list[0];
    ...
    if (waypoint_exp_id == -1) waypoint_exp_id = current_exp_id;
    return true;
}


double ExperienceMap::exp_euclidean_m(Experience *exp1, Experience *exp2)
{
    return sqrt((double)(pow((exp1->x_m - exp2->x_m),2) + pow((exp1->y_m - exp2->y_m),2)));
}
...
bool ExperienceMap::iterate()
{
    ...
    Link * link;
    /* Define the link sizes in x,y,z, alpha,beta,gamma direction */
    double lx, ly, lz, da,db,dg;
    ...
    for (i = 0; i < EXP_LOOPS; i++)
    {
        for (exp_id = 0; exp_id < experiences.size(); exp_id++)
        {
            ...
            for (link_id = 0; link_id < link_from->links_from.size(); link_id++)
            {
                ...
                // work out where e0 thinks e1 (x,y,z) should be based on the stored link information
                double elevation_angle;
                if (link->d <= 0 && link->d <= 0){
                    elevation_angle = 0;
                }
                else{
                    elevation_angle = acos(link->d/link->d); // here we compute the angle based on the height in z axis of the experience
                }
                lx = link_from->x_m + link->d * cos(link_from->gamma_m + link->heading_rad);
                ly = link_from->y_m + link->d * sin(link_from->gamma_m + link->heading_rad);
                ...
                // determine the angle between where e0 thinks e1's facing (gamma)
                // should be based on the link information
                dg = get_signed_delta_rad(link_from->gamma_m + link->gamma_rad, link_to->gamma_m);
                //cout << "dg: " << dg << endl;

                // correct e0 and e1 facing (gamma) by equal but opposite amounts
                // a 0.5 correction parameter means that e0 and e1 will be fully
                // corrected based on e0's link information
                //cout << "from 1: " << link_from->gamma_m << " to 1: " << link_to->gamma_m << endl;
                link_from->gamma_m = clip_rad_180(link_from->gamma_m + dg * EXP_CORRECTION);
                link_to->gamma_m = clip_rad_180(link_to->gamma_m - dg * EXP_CORRECTION);
                //cout << "from 2: " << link_from->gamma_m << " to 2: " << link_to->gamma_m << endl;
            }
        }
    }
...
int ExperienceMap::on_set_experience(int new_exp_id, double rel_rad_gamma)
{
    ...
    // Reset the accumulated values when a new experience has been set
    accum_delta_x = 0;
    accum_delta_y = 0;
    accum_delta_gamma = clip_rad_180(experiences[current_exp_id].gamma_m + rel_rad_gamma);

    relative_rad = rel_rad_gamma;

    return 1;
}
// create a link between two experiences
bool ExperienceMap::on_create_link(int exp_id_from, int exp_id_to, double rel_rad_gamma)
{                    
    ...
    // old heading_rad, angle around z axis (heading angle to new experience around z axis)
    new_link->heading_rad = get_signed_delta_rad(current_exp->gamma_m, atan2(accum_delta_y, accum_delta_x));
    new_link->gamma_rad = get_signed_delta_rad(current_exp->gamma_m, clip_rad_180(accum_delta_gamma + rel_rad_gamma));
 ...
 * x_m, y_m and gamma_m of the new experience are set as 0 if its the first experience
 * and set using the accum_delta values gathered sincle the last experience has been created
 */
int ExperienceMap::on_create_experience(unsigned int exp_id)
{
    ...
    //cout << "experience siz: " << experiences.size() << endl;
    ...
    {   
        ...
        new_exp->gamma_m = clip_rad_180(accum_delta_gamma);
    }
    ...
    {
        // Create a link from the current id to the last one in the experiences list
